...
const { getFirestore, collection, addDoc, doc, getDoc, query, where, getDocs } = require('firebase/firestore');
...
const firebaseConfig = {
  ...
  storageBucket: "kaarigar360.appspot.com",
...
const testBookings = [
  {
    workerId: '', // Will be filled with actual worker ID
    employerId: '', // Will be filled with actual employer ID
    status: 'completed',
...
const testRatings = [
  {
    bookingId: '', // Will be filled with actual booking ID
    workerId: '', // Will be filled with actual worker ID
    employerId: '', // Will be filled with actual employer ID
    rating: 5,
...
async function createTestData() {
  try {
    console.log('🚀 Starting to create test data for completed tasks and ratings...');

    // Get the worker ID for test2@test.com
    const usersRef = collection(db, 'users');
    const workerQuery = query(usersRef, where('email', '==', 'test2@test.com'));
    const workerSnapshot = await getDocs(workerQuery);
    
    if (workerSnapshot.empty) {
      console.error('❌ Worker with email test2@test.com not found');
      return;
    }
    ...
    console.log(`✅ Found worker: ${workerData.profile.fullName} (${workerData.email})`);
    ...
    // Note: We would need to import updateDoc for this, but for now we'll just log it
    console.log(`📊 Worker's new average rating should be: ${averageRating.toFixed(1)}`);
